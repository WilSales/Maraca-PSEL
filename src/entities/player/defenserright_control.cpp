#include "defenserright_control.h"
#include <iostream>
#include <cmath>
...
DefenserRight_Control::DefenserRight_Control(WorldMap *worldMap)
{
    _worldMap = worldMap;
    _player = nullptr;
}

void DefenserRight_Control::setPlayer(Player *player){
    _player = player;
}


void DefenserRight_Control::defend(QVector2D ballPos){
    if(_player != nullptr){
        ...
        float RightSideX = _worldMap->ourGoalRightPost().x();
        float RightSideY = _worldMap->ourGoalRightPost().y();
        float m = ( Yball - RightSideY ) / ( Xball - RightSideX );
        float Yp = Yball;
        float Xp = Xball;
        //float C = -Yp + mXp;
        //spdlog::info( "C = {}", C );
        ...
        if( delta < 0 ){
            ...
            float Ypos;
            float Xpos;
            if( Yp <= _worldMap->ourGoalRightPost().y() ){
                if( Y1 <= Y2 ){
                    Ypos = Y1;
                    Xpos = X1;
                }
...
void DefenserRight_Control::atack(int Id , QVector2D target ){

    if(_player != nullptr){
        //std::cout<<"teste"<<std::endl;
        if(Id == 5){
            //std::cout<<"id = 5"<<std::endl;
            _player->goTo( QVector2D( (_worldMap->length()/2) - (_worldMap->penaltyDepth() + 0.800f) , _worldMap->ourGoalRightPost().y() - 0.700f ) );
            ...
            if( _player->getPosition().distanceToPoint(_worldMap->ballPosition()) > ( ROBOT_RADIUS + BALL_RADIUS ) ){
                ...
                _player->goTo(QVector2D( ( _worldMap->ourGoalCenter().x() - 2.700f ), _worldMap->ourGoalRightPost().y() - 0.700f ) );
                _player->rotateTo(target);
                if( _player->getPosition().distanceToPoint(QVector2D( ( _worldMap->ourGoalCenter().x() - 2.700f ) , _worldMap->ourGoalRightPost().y() - 0.700f )) <= (ROBOT_RADIUS) ){
                    cter++;
                    if(cter >= 80){
