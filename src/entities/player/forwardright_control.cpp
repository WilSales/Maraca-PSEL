#include "forwardright_control.h"
#include <iostream>
#include <cmath>
...
ForwardRight_Control::ForwardRight_Control(WorldMap *worldMap)
{
    _worldMap = worldMap;
    _player = nullptr;
}

void ForwardRight_Control::setPlayer(Player *player){
    _player = player;
}

void ForwardRight_Control::defend(QVector2D ballPos){
    if(_player != nullptr ){
...
void ForwardRight_Control::atack(int est){
    if(_player != nullptr){
        if(est == 0){
            _player->rotateTo(_worldMap->ballPosition());
            _player->goTo(QVector2D( _worldMap->theirGoalCenter().x() + _worldMap->penaltyDepth() , _worldMap->ourGoalLeftPost().y() + 1.500f ));
        }
        else{
            _player->rotateTo(_worldMap->ballPosition());
            if( _player->getPosition().distanceToPoint(_worldMap->ballPosition()) >= (ROBOT_RADIUS + BALL_RADIUS) ){
                ...
                _player->dribble(true);
                _player->goTo(QVector2D( _worldMap->theirGoalCenter().x() + _worldMap->penaltyDepth() , _worldMap->ourGoalLeftPost().y() + 1.500f ) );
                _player->rotateTo(QVector2D(_worldMap->theirGoalRightPost().x() , _worldMap->theirGoalRightPost().y() - 0.200f ));
                if( _player->getPosition().distanceToPoint(QVector2D( _worldMap->theirGoalCenter().x() + _worldMap->penaltyDepth() , _worldMap->ourGoalLeftPost().y() + 1.500f ) ) < (ROBOT_DIAMATER)){
                    coter++;
                    if(coter >= 70){
